...
! write(6,*)Kin,Density,(Kin+MBPT_E2)/A
 write(6,*)Density,(Kin+Pot+MBPT_E2)/A,MBPT_E2
...
!** Calculating the CCD Brueckner HF energy **!
 ...
 allocate ( T_mat1(A,A,NumStates,NumStates) )
 allocate ( T_mat2(A,A,NumStates,NumStates) )
 T_mat1=0.0
 T_mat2=0.0
...
!     write(6,*)T_mat(i,j,b,c)
 ...
 do m=1,1000
  CALL CCD_Opt(L,NumStates,states,max_orbits,qnums,E,T_mat1,T_mat2)
  do i=1,A
   do j=1,A
    do b=A+1,NumStates
     do c=A+1,NumStates
  ...
  if(abs(Ec2-Ec1).lt.1.0e-8) cycle
  T_mat1=T_mat2
  Ec1=Ec2
  write(6,*)Ec2
  Ec2=0.0
...
!-------------------------------------------------------------------------
 SUBROUTINE CCD_Opt(L,NumStates,states,max_orbits,qnums,E,T1,T2)
 USE CONSTANTS
 ...
 integer states(qnums,max_orbits)
 Double precision E(max_orbits)
 Double precision T1(A,A,NumStates,NumStates),T2(A,A,NumStates,NumStates)
