...
USE CONSTANTS
 ...
 IMPLICIT NONE
	...
	Double precision, dimension(:,:,:,:), allocatable	     :: T_mat
...
! write(6,*)Kin,Density,(Kin+MBPT_E2)/A
 write(6,*)Density,(Kin+Pot+MBPT_E2)/A
...
!** Calculating the CCD Brueckner HF energy **!
 ...
 allocate ( T_mat(A,A,NumStates,NumStates) )
 T_mat=0.0
 ...
 do i=1,A
  do j=1,A
   do b=A+1,NumStates
    do c=A+1,NumStates
     T_mat(i,j,b,c)=minnesota_ass(i,j,b,c,L,states,max_orbits,qnums) &
     / (FockDiag(i) + FockDiag(j) - FockDiag(b) - FockDiag(c))
