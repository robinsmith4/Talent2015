...
! do NumPointsC=1,NumPoints
 ...
 Density=0.16*(1.0*NumPointsC/NumPoints) !fm^-3
...
!     write(6,*)vmom_minnesota(i,j,b,c,L,states,max_orbits,qnums)
     MBPT_E2=MBPT_E2+ ( (vmom_minnesota(i,j,b,c,L,states,max_orbits,qnums) &
     - vmom_minnesota(i,j,c,b,L,states,max_orbits,qnums)) &
     * (vmom_minnesota(b,c,i,j,L,states,max_orbits,qnums) &
     - vmom_minnesota(b,c,j,i,L,states,max_orbits,qnums) )  / (E(i) + E(j) - E(b) - E(c)) )
    end do
   end do
  end do
 end do
