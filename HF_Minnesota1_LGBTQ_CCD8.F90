...
USE CONSTANTS
 ...
 IMPLICIT NONE
	...
	Integer						     :: nmax,max_orbits,x,y,z,s,i,j,k,nqs,qnums,t,o,tmin,tmax
...
!** Setting up the Channels for CCD **!
 ...
 allocate ( Chan_Indx_dum(NumStates,5) )
 counter=0
 do m=-2*nmax,2*nmax
  do i=-2*nmax,2*nmax
   do j=-2*nmax,2*nmax
    do b=-2,2,2
     do c=2*tmin,2*tmax,2
      counter=counter+1
      Chan_Indx_dum(counter,1)=m
      Chan_Indx_dum(counter,2)=i
      Chan_Indx_dum(counter,3)=j
      Chan_Indx_dum(counter,4)=b
      Chan_Indx_dum(counter,5)=c
!      write(6,*)m,i,j,b,c
     end do
    end do
   end do
  ...
  do i=-2*nmax,2*nmax
   do j=-2*nmax,2*nmax
    do b=-2,2,2
     do c=2*tmin,2*tmax,2
      counter=counter+1
      Chan_Indx(counter,1)=Chan_Indx_dum(counter,1)
      Chan_Indx(counter,2)=Chan_Indx_dum(counter,2)
      Chan_Indx(counter,3)=Chan_Indx_dum(counter,3)
      Chan_Indx(counter,4)=Chan_Indx_dum(counter,4)
      Chan_Indx(counter,5)=Chan_Indx_dum(counter,5)
