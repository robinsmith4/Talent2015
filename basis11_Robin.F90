...
! write(6,*)'	state','	nx','	ny','	nz ','	spin ','	e '

 DO j=1,i
  write(6,*)j,states(1,j),states(2,j),states(3,j),states(4,j),states(5,j),E(i)
 END DO
   ...
   do j=1,A
    do gam=1,i
     do delt=1,i
      ...
      if (MATRIXELEMENT_V(m,gam,n,delt,L,states,max_orbits,qnums).ne.0.0) then
