...
!        /(FockDiag(i) + FockDiag(j) - FockDiag(a) - FockDiag(b)) !bra*ket*2.
        t2_denom(alpha)%val(bra,ket) = 1.0/(FockDiag(i) + FockDiag(j) - FockDiag(a) - FockDiag(b))
        !t2_denom(alpha)%val(bra,ket) = 1
        t2_ccm(alpha)%val(bra,ket) = t2_ccm(alpha)%val(bra,ket)*t2_denom(alpha)%val(bra,ket)
...
!  end do
 ...
 do a = below_ef+1,tot_orbs 
   do b = below_ef+1,tot_orbs
    if(ChannelCheck(alpha,a,b).ne.1) cycle
     bra = bra + 1 
     
     ket = 0 
     do i = 1, below_ef
      do j = 1, below_ef
       if(ChannelCheck(alpha,i,j).ne.1) cycle
        ket = ket + 1   
        t2_ccm_eqn(alpha)%val(bra,ket)=minnesota_ass(a,b,i,j)*t2_denom(alpha)%val(bra,ket)
        ket2 = 0 
        do c = below_ef+1,tot_orbs 
        do d = below_ef+1,tot_orbs
        if(ChannelCheck(alpha,c,d).ne.1) cycle
        ket2 = ket2 + 1 
        t2_ccm_eqn(alpha)%val(bra,ket)=t2_ccm_eqn(alpha)%val(bra,ket) &
         + 0.5*minnesota_ass(a,b,c,d)*t2_ccm(alpha)%val(ket2,ket)*t2_denom(alpha)%val(bra,ket)     
        end do
        end do
       end do
      end do
...
!     t2_ccm_eqn(alpha)%val(bra,ket)=t2_ccm_eqn(alpha)%val(bra,ket) &
!     + 0.5*vnn_pppp(alpha)%val(bra,ket2)*t2_ccm(alpha)%val(ket2,ket)*t2_denom(alpha)%val(bra,ket)
...
!     t2_ccm_eqn(alpha)%val = matmul(0.5*vnn_pppp(alpha)%val,matmul(t2_denom(alpha),t2_ccm(alpha)%val))
!     t2_ccm_eqn(alpha)%val=matmul(t2_denom(alpha)%val,t2_ccm(alpha)%val)
!   end do
!  end do
...
! Computing CCD correlation energy from initial guess of t2 (this should equal MBPT2 correction, and it does)
 CCD_E = 0.0
 do alpha = 1, Nalpha 
...
! Computing CCD correlation energy after 1 iteration. In the other code this is quite close to the MBPT2
 CCD_E = 0.0
 do alpha = 1, Nalpha 
  do bra = 1, size(vnn_hhpp(alpha)%val, 1)
   do ket = 1, size(vnn_hhpp(alpha)%val, 2)
