...
!     if(dim2.gt.0) write(6,*)'Alpha ',alpha,'Dimension 2 ',dim2
     write(6,*)'channel',alpha
     write(6,*)'dimensions',dim1,dim2
...
!		     	if(ChannelCheck(alpha,i,j).eq.1) then
!             		 ket = ket + 1
!			 vnn_pppp(alpha)%val(bra,ket) = minnesota_ass(a,b,i,j) !bra**2/ket
!	     		end if
!		end do
!           end do
!        end do
...
!                 if((ChannelCheck(alpha,i,j).eq.1).and.(bra.ne.0)) then
!		  ket = ket + 1 
!                  lookup_hh(i,j) = ket
!                  vnn_hhpp(alpha)%val(ket,bra) = minnesota_ass(a,b,i,j) !bra*ket 
!                  t2_ccm(alpha)%val(bra,ket) = minnesota_ass(a,b,i,j)/10. !bra*ket*2.
...
!	      write(6,*)alpha,bra,ket,vnn_pppp(alpha)%val(bra,ket2)*t2_ccm(alpha)%val(ket2,ket)
           end do
